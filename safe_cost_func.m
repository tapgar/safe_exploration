function [ cost ] = safe_cost_func( sf, traj, env, GP, invGP, icyGP, invicyGP, IG, s )

[ ~, cov, ~, ~, ~, newS ] = LQR_GP( env, traj, GP, invGP, icyGP, invicyGP, 100.*diag([100, 100, 100, 2, 2, 2]), eye(2));
...
%     end
    ...
    cost(i-1,1) = 10.*exp(-0.1*dx);
    
    SF = [20,1,4,400,400];
    for p = 1:1:length(IG)
        if (newS(i-1,1) == s(p,1))
            cost(i-1,1) = cost(i-1,1) + 5*(1/(1+IG(p,1)))*sqrt(sum(((newS(i-1,2:end) - s(p,2:end))./SF).^2));
        end
    end
