...
%             build straight line trajectory
        ...
        % u = [vd, thetadd]
            ...
            for i = 2:size(u,1)+1 % initial position to one beyond end of array
                ...
                q_array(i, 1) = q_array(i - 1, 1) + qd_array(i - 1, 1) * cos(q_array(i-1,3)) * obj.dt;
                q_array(i, 2) = q_array(i - 1, 2) + qd_array(i - 1, 1) * sin(q_array(i-1,3)) * obj.dt;
                % behavior for reversing
                if qd_array(i-1, 1) >= 0
                    q_array(i, 3) = q_array(i - 1, 3) + qd_array(i - 1, 2) * obj.dt;
                else
                    q_array(i, 3) = q_array(i - 1, 3) - qd_array(i - 1, 2) * obj.dt;
                end
                ...
                qd_array(i, 1) = qd_array(i - 1, 1) + u(i - 1, 1) * obj.dt;
                qd_array(i, 2) = qd_array(i - 1, 2) + u(i - 1, 2) * obj.dt;
            end
