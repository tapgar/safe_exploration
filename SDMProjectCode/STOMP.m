function u = STOMP(env, K, GP, invGP, cost_func)
    ...
    noise_factor = 100/env.POINTS_IN_TRAJ;
    ...
    while  Q_conv_counter < Q_conv % iterations over STOMP
        ...
        noise = [mvnrnd(zeros(1, POINTS_IN_TRAJ), R_1, K),mvnrnd(zeros(1, POINTS_IN_TRAJ), R_1, K),mvnrnd(zeros(1, POINTS_IN_TRAJ), R_1, K)] .* noise_factor;
        ...
        cost = zeros(K, 3*POINTS_IN_TRAJ);
        for i_K = 1:K
            traj = [env.START_STATE(1:env.TRAJ_DIMS); reshape(STOMP_traj{i_K}(:, 1),env.POINTS_IN_TRAJ,3); env.END_STATE(1:env.TRAJ_DIMS)];
            %[policy, cov, ~, ~] = LQR_GP(traj,GP,10.*eye(2),1,[]);
            pcost = cost_func(2.2, traj, env);
            cost(i_K,:) = [pcost', pcost', pcost'];
        end
        ...
        delta = zeros(POINTS_IN_TRAJ*3, 1);
        ...
        delta_smooth = reshape((M * reshape(delta,env.POINTS_IN_TRAJ,3)),env.POINTS_IN_TRAJ*3,1);
        
        u_traj_new = nom_traj + delta_smooth;
        traj = [env.START_STATE(1:env.TRAJ_DIMS); reshape(u_traj_new,env.POINTS_IN_TRAJ,3); env.END_STATE(1:env.TRAJ_DIMS)];
...
%             plot(time_array, traj, 'k', 'LineWidth', 3)
            hold on
            for i_K = 1:K
                traj2 = [env.START_STATE(1:env.TRAJ_DIMS); reshape(STOMP_traj{i_K},env.POINTS_IN_TRAJ,3); env.END_STATE(1:env.TRAJ_DIMS)];
...
%             plot(time_array, traj, 'r*', time_array, qd_targ, 'b*', time_array, qdd_targ, 'g*')
    ...
    u = [env.START_STATE(1:env.TRAJ_DIMS); reshape(u_traj_new,env.POINTS_IN_TRAJ,3); env.END_STATE(1:env.TRAJ_DIMS)];
